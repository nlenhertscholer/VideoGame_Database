-- This database will use data from the igdb website using their API.
DROP TABLE IF EXISTS game_engine;
DROP TABLE IF EXISTS genre;
DROP TABLE IF EXISTS mode;
DROP TABLE IF EXISTS published;
DROP TABLE IF EXISTS developed;
DROP TABLE IF EXISTS release_dates;
DROP TABLE IF EXISTS deployment_engines;
...
CREATE OR REPLACE FUNCTION convert_date(BIGINT)
RETURNS DATE AS $new_date$
DECLARE
    new_date DATE;
BEGIN
    SELECT (EXTRACT(YEAR FROM TO_TIMESTAMP($1))::TEXT || '-' ||
    EXTRACT(MONTH FROM TO_TIMESTAMP($1))::TEXT || '-' ||
    EXTRACT(DAY FROM TO_TIMESTAMP($1))::TEXT)::DATE into new_date;
    RETURN new_date;
END;
$new_date$ LANGUAGE plpgsql;

----------------Start declaring of tables-----------------
CREATE TABLE engine(
...
CREATE TABLE game(
  ...
  first_rel_date BIGINT     -- Using a UNIX timestamp, will convert to human readable dates when needed
);

CREATE TABLE game_engine(
  engine_id INT REFERENCES engine(engine_id) ON DELETE CASCADE,
  game_id INT REFERENCES game(game_id) ON DELETE CASCADE,
  PRIMARY KEY(engine_id, game_id)
);
...
-- Company is a publisher and/or a developer.
-- Which they are is determined by the published and developed tables
CREATE TABLE company(
  company_id INT PRIMARY KEY,
  name VARCHAR(500) NOT NULL,
  country INT, --Country code following ISO 3166-1 Standard
  founding_date BIGINT
);
...
CREATE TABLE platform(
  ...
  category_id INT REFERENCES platform_category(category_id) ON DELETE CASCADE,
  generation INT
);
...
CREATE TABLE release_dates(
  ...
  game_id INT REFERENCES game(game_id) ON DELETE CASCADE,
  rel_date BIGINT,
  category_id INT REFERENCES date_category(category_id) ON DELETE SET NULL,
  PRIMARY KEY(platform_id, game_id)
);

-- Platforms the engines are deployed on
CREATE TABLE deployment_engines(
  platform_id INT REFERENCES platform(platform_id) ON DELETE CASCADE,
  engine_id INT REFERENCES engine(engine_id) ON DELETE CASCADE,
  PRIMARY KEY (platform_id, engine_id)
);
...
INSERT INTO platform_category VALUES
(1, 'console'),
(2, 'arcade'),
(3, 'platform'),
(4, 'operating_system'),
...
INSERT INTO date_category VALUES
(0, 'YYYYMMMMDD'),
(1, 'YYYYMMMM'),
(2, 'YYYY'),
(3, 'YYYYQ1'),
...
-- Got the following table and data from the Github page:
-- https://gist.github.com/ereli/0c94ec74a1807aaa895b912766556cc2
-- This will only be used to translate country codes in the company table
